...
class Graph:
    ...
    def generate_graph(self):
        ...
        with arcpy.da.SearchCursor(self.data_fc,
                                       ["OBJECTID", "SHAPE@", 'KLASA_DROG', 'DIRECTION']) as cursor:
    ...
    def aShift8(self, cost, h, start, end):
        ...
        while Q:  # ensure loop exit when no solution
            ...
            if curr == end:
                node_path = [end]
                edge_ids = []  # create output data structures
...
def aS8_launcher(out_mode, start, end, output_name="output", in_data_fc=None, in_graph_file="graph.pkl",
...
if __name__ == '__main__':
    ...
    # curr_directory = r"C:\pw\sem5\pag2\pag_projekt_pathfinding"
    workspace = r"C:\pw\sem5\pag2\pag_projekt_pathfinding\workspace.gdb"
    ...
    action = arcpy.GetParameterAsText(1)  # [Both, Fastest_Path, Shortest_Path]
    mode = arcpy.GetParameterAsText(2)  # [Both, Create_Graph, Run_Algorithm]
    startpoint = arcpy.GetParameterAsText(3).split(',')
    endpoint = arcpy.GetParameterAsText(4).split(',')

    # input_file = 'nowy_SKJZ_L_Torun_edited'
    # action = 'Both'
    # mode = 'Both'
    # WHOLE PROCESS
    if action == 'Both':
        ...
        aS8_launcher(
            out_mode=mode,
            start=(float(startpoint[0]), float(startpoint[1])),
